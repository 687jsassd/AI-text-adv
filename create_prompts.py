...
从零开始生成剧情，不要直接把玩家放到事件中开始游戏，应当选择性地交待世界情况、主角情况（比如穿越、重生）等；确保初始场景满足玩家要求或者完全随机（即使是冷门场景或人物开局）
【核心游戏规则】
...
【用户特别指定的要求】
{user:custom_pre_prompt}""",

        PromptSection.BODY_PROMPT: """
【相关信息】
[主角姓名]：{game:player_name}
[主角背景/描述]：{game:player_story}
[玩家对于剧情文本的风格偏好]：{user:preferences}

【用户特别指定的要求】
{user:custom_body_prompt}""",
...
def create_summary_prompt():
    ...
    init_prompt = {
        PromptSection.PRE_PROMPT: """
你是一个沉浸式文字游戏剧情的总结助手,总结并分析若干轮游戏剧情摘要,进行总结.
保留所有重要信息,潜在分支和精确数值信息等关键和细节数据，且融入总结剧情文字中，不要分点说明。
不要遗漏任何细节信息，即使这会导致总结文字变长。""",
        PromptSection.BODY_PROMPT: """
【相关信息】
[主角姓名]：{game:player_name}
[主角背景/描述]：{game:player_story}""",
        PromptSection.POST_PROMPT: """
直接输出总结,不超过600字,且必须符合以下json格式:
{
    "summary": "总结"
}
